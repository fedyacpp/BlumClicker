...
class TelegramBot:
    ...
    def detect_play_button(self, screenshot, bbox, results_queue):
        ...
        for (bbox_coords, text, prob) in result:
            for keyword in self.PLAY_KEYWORDS:
                if keyword in text:
                    ...
                    self.console.log(f"'{keyword}' detected at ({x}, {y}, {w}, {h})", style="green")
                    results_queue.put((x + w // 2, y + h // 2))
                    return
        results_queue.put(None)

    def handle_predictions(self, prediction, bbox, image):
        if not prediction:
            return
        det = prediction[0]
    ...
    def run(self):
        ...
        with Live(console=self.console, refresh_per_second=self.fps_lock) as live:
            ...
            while not self.stop_signal:
                ...
                self.handle_predictions(prediction, bbox, image)
